...
def save_cnn(signals_all, signal_index, label_name):
    model = torch.load(f'{tmpdir}/alexnet-cnn-one-layer.pt')
...
def main():
    ...
    for base_model, combined_model_name in zip(base_models, combined_models_names):
        ...
        for epoch in range(num_epochs):
            ...
            if validation_accuracy > best_validation_accuracy:
                ...
                torch.save(model, f'{tmpdir}/{combined_model_name}.pt')
                print('saving as best model')
        model = torch.load(f'{tmpdir}/{combined_model_name}.pt')
        model.eval()
        test_loss_sum = 0
        corrects = 0
    ...
    os.remove(f'{tmpdir}/alexnet-cnn-one-layer.pt')
