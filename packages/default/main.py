...
class _BasicBlock(nn.Module):  # type: ignore[misc]
    def __init__(
        self: _BasicBlock,
...
class _CNNOneLayer(nn.Module):  # type: ignore[misc]
    def __init__(
        self: _CNNOneLayer,
        num_classes: int,
...
class _CNNTwoLayers(nn.Module):  # type: ignore[misc]
    def __init__(
        self: _CNNTwoLayers,
        num_classes: int,
        model_base: nn.Module,
...
class _DenseBlock(nn.Sequential):  # type: ignore[misc]
    def __init__(
        self: _DenseBlock,
        bn_size: int,
        growth_rate: int,
...
class _DenseLayer(nn.Sequential):  # type: ignore[misc]
    def __init__(
        self: _DenseLayer,
        bn_size: int,
        growth_rate: int,
...
class _DenseNet(nn.Module):  # type: ignore[misc]
    def __init__(
        self: _DenseNet,
        ...
        for index, num_layers in enumerate(block_config):
            ...
            self.features.add_module(f"denseblock{index + 1}", block)
            ...
            if index != len(block_config) - 1:
                ...
                self.features.add_module(f"transition{index + 1}", trans)
...
class _LeNet2D(nn.Module):  # type: ignore[misc]
    def __init__(self) -> None:
        super().__init__()
...
class _Lenet(nn.Module):  # type: ignore[misc]
    def __init__(self, num_classes: int) -> None:
        super().__init__()
...
class _ResNet(nn.Module):  # type: ignore[misc]
    def __init__(
        self: _ResNet,
...
class _SignalAsImage(nn.Module):  # type: ignore[misc]
    def __init__(
        self: _SignalAsImage,
        num_classes: int,
        model_base: nn.Module,
...
class _Spectrogram(nn.Module):  # type: ignore[misc]
    def __init__(
        self: _Spectrogram,
        num_classes: int,
        model_base: nn.Module,
    ...
    def forward(self, signal: torch.Tensor) -> torch.Tensor:
        ...
        for index, signal_element in enumerate(signal):
            _f_array, _t_array, spectrogram_array = spectrogram(
                signal_element.cpu(),
...
class _Transition(nn.Sequential):  # type: ignore[misc]
    def __init__(
        self: _Transition,
        num_input_features: int,
...
class _UCIEpilepsy(Dataset[tuple[torch.Tensor, torch.Tensor]]):  # type: ignore[misc]
    def __init__(
        self: _UCIEpilepsy,
        num_samples: int,
        train_validation_test: str,
    ) -> None:
        data_file_name_path = _PARENT_PATH / "prm/data.csv"
    ...
    def __len__(self) -> int:
        return self.target.shape[0]  # type: ignore[no-any-return]


class _VGG(nn.Module):  # type: ignore[misc]
    def __init__(self, num_classes: int, features: nn.Module) -> None:
        super().__init__()
...
def main() -> None:  # noqa: C901,PLR0912,PLR0915
        ...
        for model_module_name_index, model_module_name in enumerate(
            ...
            for _ in range(num_epochs):
                ...
                if accuracy_validation > accuracy_validation_best:
                    ...
                    model_file_name = _OUT_PATH / f"{model_name}.pt"
    ...
    styler.to_latex(_OUT_PATH / "results.tex", hrules=True)
    ...
    for class_index, class_name in enumerate(class_names):
        ...
        plt.figure()
        plt.imsave(
            _OUT_PATH / f"signal-as-image-{class_name}.png",
            data,
            cmap="gray",
        )
        plt.close()
        ...
        plt.figure()
        plt.imsave(
            _OUT_PATH / f"spectrogram-{class_name}.png",
            data,
            cmap="gray",
        )
        plt.close()
        ...
        model.load_state_dict(
            torch.load(
                _OUT_PATH / "alexnet-cnn-one-layer.pt",
                weights_only=True,
            ),
        )
